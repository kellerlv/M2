...
adjoinVariables (DGAlgebra, List) := opts -> (A, cycleList) -> (
   ...
   -- Use degree z + tempDegree uniformly: this works whether A is
   -- internally homogeneous or not. In the !isHomogeneous case the
   -- "internal degree" of z is whatever M2 returns (typically the
   -- max-component or first-term degree), but the result still has the
   -- right length to concatenate with A.Degrees. Falling back to a
   -- length-1 {first deg z + 1} (as the original code did) introduces
   -- a length mismatch when A.Degrees already has length-2 entries
   -- from a bi-graded koszulComplexDGA construction.
