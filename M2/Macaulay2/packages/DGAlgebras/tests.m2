...
-- Test: the HMF Omega^3 regression case. Original algorithm gave 48
-- generators (entry-non-minimal); patched gives 46 = exact match to
-- Tor^S(M3, k) = {6, 9, 13, 18}.
TEST ///
needsPackage "DGAlgebras"
kk = ZZ/101
P = kk[a, b, c]
...
F = minimalSemifreeResolution(A, MR, EndDegree => 2)
ranks = apply(max(F.Degrees / first) + 1, d -> #select(F.Degrees, e -> first e == d))
assert(ranks == {6, 9, 13, 18})
assert(isMinimalSemifreeResolution F)
-- Without post-processing we get the older (non-minimal) ranks.
Fraw = minimalSemifreeResolution(A, MR, EndDegree => 2, Minimize => false)
rawRanks = apply(max(Fraw.Degrees / first) + 1, d -> #select(Fraw.Degrees, e -> first e == d))
assert(rawRanks == {6, 10, 14, 18})  -- pre-fix behavior, 2 extra contractible gens
-- And the unminimized output is still acyclic (just has scalar units).
assert(not isMinimalSemifreeResolution Fraw)
-- Minimizing the raw output gives the same final ranks as direct call.
Fmin = minimizeDGModule Fraw
minRanks = apply(max(Fmin.Degrees / first) + 1, d -> #select(Fmin.Degrees, e -> first e == d))
assert(minRanks == {6, 9, 13, 18})
///
...
-- Test: a deep HMF case (Omega^9) — exercises a more complex semifree
-- resolution structure and confirms ranks still match Tor^Q exactly.
TEST ///
needsPackage "DGAlgebras"
kk = ZZ/101
P = kk[a, b, c]
